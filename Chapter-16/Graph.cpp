...
namespace Graph_lib {
	...
	Func::Func(Fct fn, double min, double max, Point orig, int pts_num, double xscale, double yscale, double precision)
		:Function(fn, min, max, orig, pts_num, xsc, ysc),
		f(fn), r1(min), r2(max), xy(orig), count(pts_num), xsc(xscale), ysc(yscale), prec(precision)
	{
		restore();	
	}
	void Func::set_function(Fct fn)
	{
		f = fn;
		restore();
	}
	void Func::set_range(int min, int max)
	{
		if (max - min <= 0) error("bad graphing range");
		r1 = min;
		r2 = max;
		restore();
	}
	void Func::set_xy(Point orig)
	{
		xy = orig;
		restore();
	}
	void Func::set_points_num(int pts_num)
	{
		if(pts_num < 1) error("non-positive graphing count");
		count = pts_num;
		restore();
	}
	void Func::set_scales(int xscale, int yscale)
	{
		xsc = xscale;
		ysc = yscale;
		restore();
	...
	void Func::restore()
	{
		if (count < 1) error("non-positive graphing count");
		if (r2 - r1 <= 0) error("bad graphing range");
		...
		double r = r1;
		for (int i = 0; i < count; ++i) {
			add(Point(xy.x + int(r*xsc/prec)*prec, xy.y - int(f(r)*ysc/prec)*prec));
			r += dist;
		}
	}

	Bar_graph::Bar_graph(Point xy, string graph_label, vector<double> values, int max_length, int bar_width, int space)
		:label(Point(xy.x, xy.y - 5), graph_label), length(max_length), sp(space)
	{
		double max_val = 0;
		for (int i = 0; i < values.size(); ++i)
		{
			if (max_val <= abs(values[i])) max_val = abs(values[i]);
		}

		for (int i = 0; i < values.size(); ++i)
		{
			Point sp(xy.x, xy.y + i * (bar_width + space));
			int bar_length = int(abs(values[i]) / max_val * max_length);
			if (values[i] <= 0)
			{
				rects.push_back(new Rectangle(Point(sp.x - bar_length, sp.y), bar_length+1, bar_width)); // +1 to avoid 0 as length of rectangle
				labels.push_back(new Text(Point(sp.x + space, sp.y + bar_width), ""));
			}
			else
			{
				rects.push_back(new Rectangle(sp, bar_length, bar_width));
				labels.push_back(new Text(Point(sp.x + bar_length + space, sp.y + bar_width), ""));
			}
		}
	}
	...
	void Bar_graph::set_color(Color c)
	{
		...
		for (int i = 0; i < rects.size(); ++i)
		{
			rects[i].set_fill_color(c);
			labels[i].set_color(c);
		}
	}
	void Bar_graph::move_bar(int i, Point pos)
	{
		int dx = -(rects[i].point(0).x - pos.x);
		int dy = -(rects[i].point(0).y - pos.y);
		rects[i].move(dx,dy);
		labels[i].move(dx,dy);
	}
	...
	*/
	...
	Clock::Clock(Point xy, int rr)
		:Circle(xy, rr),
		hh(0), mm(0), ss(0)
	{
		Circle::set_style(Line_style(Line_style::solid, 6));
		ticks.set_style(Line_style(Line_style::solid, 4));
		for (double ang = 0; ang < 2 * PI; ang += PI / 6)
		{
			ticks.add(Point(xy.x + 0.9*rr*cos(ang), xy.y + 0.9*rr*sin(ang)),
				Point(xy.x + rr*cos(ang), xy.y + rr*sin(ang)));
		}
		...
		mm = now->tm_min;
		ss = now->tm_sec;
		hands.push_back(new Line(xy,
			Point(xy.x + 0.6*rr*cos(PI / 2 - 2 * PI / 12 * hh), xy.y - 0.6*rr*sin(PI / 2 - 2 * PI / 12 * hh))));
		hands[0].set_style(Line_style(Line_style::solid, 12));
		hands.push_back(new Line(xy,
			Point(xy.x + 0.85*rr*cos(PI / 2 - 2 * PI / 60 * mm), xy.y - 0.85*rr*sin(PI / 2 - 2 * PI / 60 * mm))));
		hands[1].set_style(Line_style(Line_style::solid, 8));
		hands.push_back(new Line(xy,
			Point(xy.x + 0.9*rr*cos(PI / 2 - 2 * PI / 60 * ss), xy.y - 0.9*rr*sin(PI / 2 - 2 * PI / 60 * ss))));
		hands[2].set_style(Line_style(Line_style::solid, 2));
		hands[2].set_color(Color::red);
	}
	void Clock::update() {
		hands[0].set_point(1, Point(center().x + 0.6*radius()*cos(PI / 2 - 2 * PI / 12 * hh),
			center().y - 0.6*radius()*sin(PI / 2 - 2 * PI / 12 * hh)));
		hands[1].set_point(1, Point(center().x + 0.85*radius()*cos(PI / 2 - 2 * PI / 60 * mm),
			center().y - 0.85*radius()*sin(PI / 2 - 2 * PI / 60 * mm)));
		hands[2].set_point(1, Point(center().x + 0.9*radius()*cos(PI / 2 - 2 * PI / 60 * ss),
			center().y - 0.9*radius()*sin(PI / 2 - 2 * PI / 60 * ss)));
	}
